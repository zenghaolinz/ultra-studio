...
from schemas import ChatRequest, ConversationCreate, ProjectCreate
...
def _is_docx_create_intent(content: str) -> bool:
...
def _is_text_file_create_intent(content: str) -> bool:
    text = (content or "").lower()
    if _is_docx_create_intent(content):
        return False
    ...
    return any(word in text for word in create_words) and any(
...
def _needs_implementation_choice(content: str) -> bool:
    if not _is_game_file_request(content):
        return False
    return not _has_explicit_text_file_format(content)


def _format_implementation_choice_card(content: str) -> str:
    return "\n".join(
        [
...
async def _run_direct_text_file_create(
    req: ChatRequest,
    client,
    model_name: str,
    force: bool = False,
    ...
    if req.image_paths or (not force and not _is_text_file_create_intent(request_text)):
        return None
    ...
    payload = _router_safe_json(response.choices[0].message.content or "{}")
...
def _is_text_file_edit_followup_intent(content: str) -> bool:
    text = (content or "").lower()
    blocked_words = [
        "删除",
        "删掉",
...
def _extract_explicit_text_file_path(content: str) -> str | None:
...
async def _find_latest_text_file_path(conversation_id: str) -> str | None:
    db = await get_db()
...
async def _run_direct_text_file_edit(req: ChatRequest, client, model_name: str) -> dict | None:
    if req.image_paths:
        return None

    target_path = _extract_explicit_text_file_path(req.content)
    if not target_path:
        target_path = await _find_latest_text_file_path(req.conversation_id)
    if not target_path or not _is_text_file_edit_followup_intent(req.content):
        return None
...
def _edit_text_result_can_fallback(result: dict | None) -> bool:
...
def _is_docx_edit_intent(content: str) -> bool:
    text = (content or "").lower()
...
def _is_docx_followup_edit_intent(content: str) -> bool:
    ...
    return any(word in text for word in followup_words) and not any(
        word in text for word in blocked_words
    )
...
async def _run_direct_docx_edit(req: ChatRequest, client, model_name: str) -> dict | None:
    if req.image_paths:
        return None
    ...
    if not (_is_docx_edit_intent(req.content) or (target_path and _is_docx_followup_edit_intent(req.content))):
        return None
    if not target_path:
        return None
...
async def _run_direct_docx_create(req: ChatRequest, client, model_name: str) -> dict | None:
    if req.image_paths or not _is_docx_create_intent(req.content):
        return None
...
async def _run_direct_document_read(req: ChatRequest, client, model_name: str) -> str | None:
    ...
    for path in docs[:3]:
        ...
        if not result.get("ok"):
            ...
            continue
        sections.append(
            f"[{result.get('name') or path}]\n{result.get('content', '')}"
        )

    system_hint = (
        "你是文档阅读助手。基于用户提供的文档内容回答，不要编造。"
        "如果内容不足，直接说明。回答要简洁、结构清楚。"
    )
    user_text = (
        f"用户需求：{req.content}\n\n"
        f"文档内容：\n\n" + "\n\n---\n\n".join(sections)
...
def _format_docx_create_response(result: dict) -> str:
    if result.get("ok"):
        return f"已创建 Word 文档：`{result.get('path')}`"
    return f"创建 Word 文档失败：{result.get('error', '未知错误')}"


def _format_docx_edit_response(result: dict) -> str:
    if result.get("ok"):
...
def _format_text_file_create_response(result: dict) -> str:
    if not result.get("ok"):
